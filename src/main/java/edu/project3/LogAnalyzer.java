...
import edu.project3.parser.argsparser.ArgsParser;
...
public class LogAnalyzer {
    ...
    public Report analyze(String[] args) {
        ParseFormat parseFormat = ArgsParser.getParseFormat(String.join(" ", args));
        Receiver receiver;
        ...
        if (parseFormat.path().startsWith("http")) {
            ...
            List<Path> pathsToLogs = PathParser.getPaths(parseFormat.path());
            paths = pathsToLogs.stream()
                .map(Path::toString)
                .toList();
        ...
        return new Report(parseFormat.format(), metrics);
    }
    ...
    private List<Metric> getAllMetrics(List<Log> logs, List<String> paths) {
        MetricBuilder metricMainInfoBuilder = new MetricMainInfoBuilder(fromDateOffset, toDateOffset, paths);
